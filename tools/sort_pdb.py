...
ROOT = "/vepfs-mlp2/mlp-public/shikunfeng/Datas/PDBBIND_atomCorrected"
...
def process_one_complex(folder):
    ...
    for f in os.listdir(subdir):
        if f.endswith("_protein_processed_fix.pdb"):
            exp_path = os.path.join(subdir, f)
        if f.endswith("_protein_esmfold_aligned_tr_fix.pdb"):
