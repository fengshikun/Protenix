...
## 2026-03-05 训练补充记录（W&B + OOM + 日志降噪）

### 1) W&B 卡住原因

- 现象：终端重复出现 `wandb: Enter your choice:`，训练不继续。
- 根因：W&B 在等待交互式登录，不是训练代码卡死。
- 处理：
  - 在线模式：先登录
    ```bash
    export WANDB_API_KEY="<your_key>"
    conda run -n protenix311 wandb login --relogin "$WANDB_API_KEY"
    ```
  - 临时不登录：`WANDB_MODE=offline` 或 `--use_wandb False`

### 2) 联合训练（contrast + protenix）OOM 记录

- 报错：`torch.OutOfMemoryError` 出现在 `backward()`，rank2 申请 3.76GiB 失败。
- 原因：从 `train_projection_only=True` 的轻量路径切换到联合反传后，显存占用显著增加。
- 稳定配置建议：
  - 每卡 batch 改为 2：`--batch_size 2 --data.batch_size 2`
  - 保持有效 batch：`--iters_to_accumulate 4`
  - 降低碎片化：
    ```bash
...
- 当前代码只在满足以下条件时 `wandb.log`：
  - 到达更新步（`is_update_step`）
  - 且 `(step + 1) % log_interval == 0`
- 首个点出现批次数：
  - `log_interval * iters_to_accumulate`
  - 例如 `log_interval=10, iters_to_accumulate=1`，大约第 10 个 batch 后出现。
...
- 修改文件：
  - `protenix/model/loss.py`
    - `DDBM MSELoss before scaling` 改为 `logging.debug`
    - `DDBM MSELoss` 改为 `logging.debug`
    - `loss_name ... weight ...` 与 `weighted_xxx` 改为 `logging.debug`
    - 异常场景保留为 `logging.warning`
  - `protenix/model/modules/diffusion.py`
    - `Diffusion Module has ...` 改为 `logging.debug`
- 结果：默认 `INFO` 级别下不再刷这些调试行。
- 结果摘要（关键对比）：
...
## 本轮问题排查与修复（2026-03-05）

### 问题 1：`--contrast.only_forward` / `--contrast.train_projection_only` 报 `unrecognized arguments`

- 现象：
  - 运行命令中传入 `--contrast.only_forward False --contrast.train_projection_only True` 后，`train.py` 直接参数解析失败。
- 根因：
  1. `runner/train.py` 先按基础配置构建 argparse，再加载 `model_name` 对应配置，导致首次解析时 `contrast.*` 尚未注册。
  2. 模型配置是 `ConfigDict`，若直接 `update`，会在参数层表现为 `--contrast` 整体键，而不是 `--contrast.xxx` 子键。
- 修改位置：
  - `runner/train.py`（`main()`）
- 修改内容：
  1. 先从 `sys.argv` 读取 `--model_name`，先加载模型专属配置，再统一 parse。
  2. 模型配置转为普通 `dict` 并做深合并，确保 `contrast.*` 子键被注册。
- 验证方式：
  - 最小命令复现后，不再出现 `unrecognized arguments`，程序可进入数据集加载阶段。

### 问题 2：修复问题 1 后出现 `KeyError/AttributeError: use_apo_pos`

- 现象：
  - 参数可识别后，初始化 dataset 时出现：`configs.model.diffusion_module.use_apo_pos` 缺失。
- 根因：
  - 对模型配置做了浅层 `update`，把 `model` 下的默认嵌套字段覆盖丢失。
- 修改位置：
  - `runner/train.py`（`main()`）
- 修改内容：
  - 新增 `_deep_update(dst, src)`，把 `model_configs[model_name]` 深合并进基础配置，保留默认字段。
- 验证方式：
  - 同一最小命令下，不再出现 `use_apo_pos` 错误，能继续初始化 dataloader。

### 问题 3：想加结构损失反传，但实际仍是纯对比训练

- 现象：
  - 你期望“只训练 MLP + 同时回传结构损失”，但训练行为仍偏向纯 contrast。
- 根因：
  1. `v0.2.7` 默认 `contrast.only_forward=True`（会跳过结构主干输出路径）。
  2. 命令里把 `alpha_diffusion/alpha_distogram/alpha_confidence` 设为 0，会把结构损失权重清空。
- 正确做法：
  - 保留 `--contrast.train_projection_only True`（只训投影头）
  - 显式设置 `--contrast.only_forward False`
  - 把结构损失权重设回非零

### 问题 4：`--data.test_sets null` 引发数据集 KeyError

- 现象：
  - 报错 `KeyError: 'null'` 或 `argument --data.test_sets: expected one argument`。
- 根因：
  - 参数系统把 `null` 当成字符串键名，而不是空值。
- 处理建议：
  - 不要传 `--data.test_sets null`。
  - 若需关闭评估，直接把 `--eval_interval` 设很大（你已这么做）。

## 当前推荐运行命令（4 卡，结构+对比联合；仅训练 MLP 头）

> 说明：该命令基于以上修复后的代码，支持 `--contrast.only_forward/--contrast.train_projection_only`。

```bash
cd /home/dataset-local/tmp/zsl/Protenix && \
export PROTENIX_DATA_ROOT_DIR=/home/dataset-local/tmp/zsl/Protenix/release_data && \
export CC=/usr/bin/gcc CXX=/usr/bin/g++ && \
export TORCH_NCCL_ASYNC_ERROR_HANDLING=1 && \
export NCCL_DEBUG=warn NCCL_P2P_DISABLE=1 NCCL_IB_DISABLE=1 && \
CUDA_VISIBLE_DEVICES=0,1,2,3 conda run --no-capture-output -n protenix311 \
torchrun --standalone --nproc_per_node=4 runner/train.py \
  --model_name protenix_mini_esm650m_unimol_contrast_v0.2.7 \
  --project debug_qbiolip \
  --run_name DDP4-contrast-structure-mlp-only-v027 \
  --base_dir ./output \
  --batch_size 8 \
  --data.batch_size 8 \
  --data.num_dl_workers 0 \
  ...
  --chain_permutation.train.mini_rollout False \
  --atom_permutation.train.mini_rollout False \
  --contrast.only_forward False \
  --contrast.train_projection_only True \
...
## 同命令迁移路径版本（如在 `/home/dataset-assist-0/...` 运行）

```bash
cd /home/dataset-assist-0/tmp/zsl/zsl/Protenix && \
export PROTENIX_DATA_ROOT_DIR=/home/dataset-assist-0/tmp/zsl/zsl/Protenix/d_data && \
export CC=/usr/bin/gcc CXX=/usr/bin/g++ && \
export TORCH_NCCL_ASYNC_ERROR_HANDLING=1 && \
export NCCL_DEBUG=warn NCCL_P2P_DISABLE=1 NCCL_IB_DISABLE=1 && \
CUDA_VISIBLE_DEVICES=0,1,2,3 conda run --no-capture-output -n protenix311 \
...
## 2026-03-05 v0.5 权重迁移（已加载 Protenix checkpoint）

- 迁移目标：
  - 在 `protenix_mini_esm650m_unimol_contrast_v0.2.7` 上初始化加载
    `/home/dataset-local/tmp/zsl/Protenix/release_data/checkpoint/protenix_mini_esm_v0.5.0.pt`
  - 采用参数迁移微调（不恢复旧优化器与 step）。

### 本地路径命令（联合训练 + 加载 v0.5 权重）
...
torchrun --standalone --nproc_per_node=4 runner/train.py \
  --model_name protenix_mini_esm650m_unimol_contrast_v0.2.7 \
  --project debug_qbiolip \
  --run_name DDP4-joint-contrast-protenix-v027-from-v050 \
  --base_dir ./output \
  --batch_size 2 \
  --data.batch_size 2 \
  --data.num_dl_workers 0 \
  ...
  --loss.weight.alpha_distogram 0.03 \
  --loss.weight.alpha_confidence 1e-4 \
  --load_checkpoint_path /home/dataset-local/tmp/zsl/Protenix/release_data/checkpoint/protenix_mini_esm_v0.5.0.pt \
...
- 备注：
  - `--load_params_only True` = 只迁移模型参数，不恢复 optimizer/scheduler/step。
  - 若需要“断点续训”而非“迁移微调”，改为：
    - `--load_params_only False`
    - 并显式设置 `--skip_load_optimizer False --skip_load_scheduler False --skip_load_step False`

## 2026-03-05 Benchmark 推理评测流程（lit_pcba + DUD-E）

### 新增脚本

- `scripts/benchmark/build_lit_pcba_lmdb.py`
  - 把 lit_pcba 原始文件（`actives.smi/inactives.smi + *_protein.mol2/*_ligand.mol2`）转换为：
    - `mols.lmdb`
    - `pockets.lmdb`
- `scripts/benchmark/eval_protenix_vs.py`
  - 支持 checkpoint 分步评测并输出：
    - `benchmark_summary.csv`
    - `per_checkpoint/<step>.json`
    - `curve_dude.png`
    - `curve_lit_pcba.png`
  - 已支持增量评测：若 `per_checkpoint/<step>.json` 已存在则自动跳过（可用 `--force_recompute` 强制重算）
...
python scripts/benchmark/build_lit_pcba_lmdb.py \
  --lit_pcba_root /home/dataset-local/tmp/zsl/Protenix/benchmark/lit_pcba \
  --num_workers 16 \
  --force
```

### DUD-E 转换命令（DrugCLIP 脚本）

```bash
cd /home/dataset-local/tmp/zsl/Protenix
conda run --no-capture-output -n protenix311 pip install biopandas
conda run --no-capture-output -n protenix311 \
python DrugCLIP/py_scripts/write_dude_multi.py \
  --mol_data_path /home/dataset-local/tmp/zsl/Protenix/benchmark/DUD-E/raw/all
```

### 按训练步数评测命令（每 2000 step）

```bash
cd /home/dataset-local/tmp/zsl/Protenix
conda run --no-capture-output -n protenix311 \
python scripts/benchmark/eval_protenix_vs.py \
  --model_name protenix_mini_esm650m_unimol_contrast_v0.2.7 \
  --checkpoint_dir /home/dataset-local/tmp/zsl/Protenix/output/<run_name>/checkpoints \
  --step_mod 2000 \
  --dude_root /home/dataset-local/tmp/zsl/Protenix/benchmark/DUD-E/raw/all \
  --pcba_root /home/dataset-local/tmp/zsl/Protenix/benchmark/lit_pcba \
  --lig_batch_size 64 \
  --device cuda:0 \
  --output_dir /home/dataset-local/tmp/zsl/Protenix/output/<run_name>/benchmark_eval
```

### 训练中周期性增量评测（示例：每 30 分钟）
...
- `DUD-E`: 还未准备（target=0）
- `lit_pcba`: 15 个 target 中，已有 1 个 target 具备 lmdb，剩余 14 个待转换

## 2026-03-06 Benchmark 评测补充记录（ESM+UniMol 输入适配）

### 评测脚本关键变更

- 文件：`scripts/benchmark/eval_protenix_vs.py`
- 变更 1：`lit_pcba` 默认不强依赖 `pockets.lmdb`
  - 评测时默认仅要求 `mols.lmdb`（蛋白序列来自 `*_protein.mol2` 解析后走 ESM）
  - 新增开关：`--pcba_require_pocket`
  - 仅在显式加该开关时才要求 `pockets.lmdb`
- 变更 2：修复 UniMol 配体输入类型
  - 之前报错：
    - `ValueError: Type must be a sub-type of ndarray type`
  - 修复方式：
    - 在 `encode_ligands` 中将 `coordinates` 转为 `torch.float32` tensor 后再喂 `load_mols_dataset`
  - 目的：
    - 与 Protenix 训练路径中的 UniMol 数据格式保持一致

### lit_pcba smoke 推理（已跑通）

- 运行时间：`2026-03-05 21:12` ~ `21:16`（Asia/Shanghai）
- 命令（单 checkpoint + 单 target）：

```bash
conda run --no-capture-output -n protenix311 \
python /home/dataset-local/tmp/zsl/Protenix/scripts/benchmark/eval_protenix_vs.py \
  --model_name protenix_mini_esm650m_unimol_contrast_v0.2.7 \
  --checkpoint_paths /home/dataset-local/tmp/zsl/Protenix/output/DDP4-contrast-drugclipmask-v027-20k_20260305_025910/checkpoints/1999.pt \
  --pcba_root /home/dataset-local/tmp/zsl/Protenix/benchmark/lit_pcba \
  --skip_dude \
  --limit_targets 1 \
  --lig_batch_size 64 \
  --device cuda:0 \
  --output_dir /home/dataset-local/tmp/zsl/Protenix/output/DDP4-contrast-drugclipmask-v027-20k_20260305_025910/benchmark_eval_litpcba_smoke2 \
  --log_level INFO
```

- 实际输出（ADRB2）：
  - `AUCROC = 0.4100471343661238`
  - `BEDROC_80.5 = 5.721574589069041e-07`
  - `EF0.5% = 0.0`
  - `EF1% = 0.0`
  - `EF2% = 0.0`
  - `EF5% = 0.0`
...
- 汇总：`/home/dataset-local/tmp/zsl/Protenix/output/DDP4-contrast-drugclipmask-v027-20k_20260305_025910/benchmark_eval_litpcba_smoke2/benchmark_summary.csv`
- 细节：`/home/dataset-local/tmp/zsl/Protenix/output/DDP4-contrast-drugclipmask-v027-20k_20260305_025910/benchmark_eval_litpcba_smoke2/per_checkpoint/1999.json`
- 曲线：`/home/dataset-local/tmp/zsl/Protenix/output/DDP4-contrast-drugclipmask-v027-20k_20260305_025910/benchmark_eval_litpcba_smoke2/curve_lit_pcba.png`
